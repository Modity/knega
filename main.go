...
func main() {
	...
	*/
	...
	app.Commands = []cli.Command{
		...
		{
			Name:  "single",
			Usage: "single <action> will run action for single applications",
			Subcommands: []cli.Command{
				{
					Name:  "check",
					Usage: "Run build command defined in application configs where changes have occurred",
					Action: func(context *cli.Context) error {
						return single("check", context.Args().Get(0))
					},
				},
				{
					Name:  "build",
					Usage: "Run build command defined in application configs where changes have occurred",
					Action: func(context *cli.Context) error {
						return single("build", context.Args().Get(0))
					},
				},
				{
					Name:  "analyze",
					Usage: "Run analyze command defined in application configs where changes have occurred",
					Action: func(context *cli.Context) error {
						return single("analyze", context.Args().Get(0))
					},
				},
				{
					Name:  "release",
					Usage: "Run release command in all applications, passing in $INPUTS_HASH",
					Action: func(context *cli.Context) error {
						return single("release", context.Args().Get(0))
					},
				},
				{
					Name:  "migrate",
					Usage: "Run migrate command in all applications, passing in $INPUTS_HASH",
					Action: func(context *cli.Context) error {
						return single("migrate", context.Args().Get(0))
					},
				},
				{
					Name:  "postrelease",
					Usage: "Run post release command in all applications, passing in $INPUTS_HASH",
					Action: func(context *cli.Context) error {
						return single("postrelease", context.Args().Get(0))
					},
				},
			},
		},
